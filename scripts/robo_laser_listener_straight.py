...
def changeAngle(current_angle, destination_angle, change_rate):
    if  current_angle < destination_angle + (change_rate+2) and angz > destination_angle - (change_rate+2):
...
def changeSpeed(current_speed, destination_speed, change_rate):
    if  current_speed < destination_speed + (change_rate+2) and angz > destination_speed - (change_rate+2):
...
def LaserScanProcess(data):
    ...
    if average_gap < max_gap:   # path block
        ...
        angz = changeAngle(angz, -max_angle, 1)
        #y = data.ranges[len(data.ranges)/2]
        y = min(data.ranges)
        LINX = 0.125*max_speed*sigmoid(y)

        print(angz,"********")
    else: 
        """
        ang_want = Kp*(90 - turn_angle)
        if  angz < ang_want + 5 and angz > ang_want - 5:
            angz = angz
        elif angz < ang_want:
            angz = angz + 3
        elif angz > ang_want:
        ...
        if (angz > max_angle):
                angz = max_angle        
        """
        angz = changeAngle(angz, Kp*(90 - turn_angle),1)        
        y = data.ranges[len(data.ranges)/2]
        LINX = LINX+max_speed*sigmoid(y)
    if LINX > max_speed:
        LINX = max_speed
    if LINX < 20:
        LINX = 0
        """
        if angz < 5 and angz > -5:
            angz = 0
        elif (angz >= 5):
            angz = angz - 3
        else:
